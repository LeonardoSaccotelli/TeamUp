			...
			<?php if ( true == $args['show_actions'] ) { ?>
				<div class="actions">
					<ul>
						...
						<li class="last-active"><?php echo esc_html( __( 'Ultima attività: ', 'um-groups' ) . human_time_diff( UM()->Groups()->api()->get_group_last_activity( $group->ID, true ), current_time( 'timestamp' ) ) . __( ' fa', 'um-groups' ) ); ?></li>
						<li class="count-members"><?php echo sprintf( _n( '<span>%s</span> Teammate', '<span>%s</span> members', $count, 'um-groups' ), number_format_i18n( $count ) ); ?></li>
