	...
	foreach( $groups as $group ) :
		...
		<div class="um-group-item">
			...
			<?php if ( true == $args['show_actions'] ) { ?>
				<div class="actions">
					<ul>
						<li><a href="<?php echo esc_url( $profile_url ) ?>" class="um-left"><?php echo $profile_avatar; ?></a><?php esc_html_e( 'Sei stato invitato da ', 'um-groups' ); ?> <br><a href="<?php echo esc_url( $profile_url ); ?>"><?php echo esc_html( $profile_name ); ?></a></li>
						<li><?php echo esc_html( __( 'Would you like to join this group?', 'um-groups' ) ); ?></li>
						<li>
							<div class="um-groups-double-button">
								<a href="javascript:void(0);" data-group_id="<?php echo esc_attr( $group->group_id ); ?>" data-user_id="<?php echo esc_attr( $user_id ); ?>" class="um-button um-groups-confirm-invite" ><?php echo esc_html( __( "Conferma invito", "um-groups" ) ); ?></a>
								<a href="javascript:void(0);" data-group_id="<?php echo esc_attr( $group->group_id ); ?>" data-user_id="<?php echo esc_attr( $user_id ); ?>" class="um-button um-groups-ignore-invite um-alt" ><?php echo esc_html( __( "Ignora invito", "um-groups" ) ); ?></a>
							</div>
						</li>
					</ul>
				</div>
			...
			<div class="um-group-meta">
				<ul>
					<li class="privacy">
						...
						<?php printf( __( '%s Project', 'um-groups' ), um_groups_get_privacy_title( $group->ID ) ); ?>
					</li>
