...
<div id="um-groups-filters" class="um-groups-found-posts">
	<ul class="filters">
		<li class="all <?php echo esc_attr( 'all' == $filter || empty( $filter ) ? 'active' : '' ); ?>">
			<a href="<?php esc_attr_e( um_get_core_page( 'groups' ) ) ?>"><?php printf( __( 'Tutti i progetti <span>%s</span>', 'um-groups' ), um_groups_get_all_groups_count() ) ?></a>
		</li>

		<?php if( is_user_logged_in() ) : ?>
			<li class="own <?php echo esc_attr( 'own' == $filter ? 'active' : '' ); ?>">
				<a href="<?php esc_attr_e( um_get_core_page( 'groups' ) ) ?>?filter=own"><?php printf( __( 'I miei progetti <span>%s</span>', 'um-groups' ), um_groups_get_own_groups_count() ) ?></a>
			</li>
			<li class="create">
				<a href="<?php esc_attr_e( um_get_core_page( 'create_group' ) ) ?>"><?php _e( 'Crea un progetto', 'um-groups' ) ?></a>
			</li>
		<?php endif; ?>
	</ul>
</div>
