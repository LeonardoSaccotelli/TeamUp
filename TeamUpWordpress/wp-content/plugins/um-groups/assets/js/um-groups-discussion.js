		...
		// if we are writing a new reply
			...
			jQuery.ajax({
				...
				success: function (data) {
					...
					jQuery( 'body' ).scrollTo( clonel, 500, {
						offset: -32
					} );
...
jQuery(document).ready(function () {
	...
	jQuery( document ).on( 'keypress', '.um-groups-comment-textarea', function (e) {
		...
		e.preventDefault();
		var $textarea = jQuery( e.currentTarget ).closest( '.um-groups-comment-area' ).find( '.um-groups-comment-textarea.ui-autocomplete-input:visible' ).last();
